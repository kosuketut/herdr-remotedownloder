...
拡張子に関係なく通常のファイルを転送します。ディレクトリは自動的に
アーカイブされ、転送先で展開されます。逆方向はMacで選んだファイルを、
...
- ディレクトリは自動的に `.tar.gz` にアーカイブされ、ダウンロード先の新規フォルダー内で展開されます。アーカイブ内のシンボリックリンクや特殊ファイルはスキップされます。
- 既定では1ファイルあたり512 MiBを超えるファイルを転送できません。接続元と接続先の両方で `HERDR_DOWNLOAD_MAX_MB`（例: `2048`、`0` で無制限）を設定するか、`--max-mb` オプションを使用してください。
- 転送サービスの自動起動はmacOSではlaunchd、Linuxではsystemdユーザーサービスに対応しています。
